...
router.post('/', (0, auth_1.requireRole)('ADMIN'), async (req, res) => {
    try {
        ...
        const passwordHash = await (await Promise.resolve().then(() => __importStar(require('bcryptjs')))).default.hash(input.password, 10);
...
router.get('/', (0, auth_1.requireRole)('ADMIN'), async (req, res) => {
    const users = await prisma_1.prisma.user.findMany({ where: { companyId: req.user.companyId } });
    res.json(users);
});

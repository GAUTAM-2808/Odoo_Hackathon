...
exports.buildApprovalChainForExpense = buildApprovalChainForExpense;
exports.handlePostApprovalProgress = handlePostApprovalProgress;
const prisma_1 = require("../prisma");
async function buildApprovalChainForExpense(expense) {
    // Build chain by walking manager hierarchy up to ADMIN
    const chain = [];
    let submitter = await prisma_1.prisma.user.findUnique({ where: { id: expense.userId } });
    const seen = new Set();
    while (submitter?.managerId) {
        if (seen.has(submitter.managerId))
            break; // safety against cycles
        const manager = await prisma_1.prisma.user.findUnique({ where: { id: submitter.managerId } });
        if (!manager)
            break;
        chain.push(manager);
    ...
    const approvals = await prisma_1.prisma.$transaction(chain.map((user, index) => prisma_1.prisma.approval.create({
        data: {
...
async function handlePostApprovalProgress(expenseId) {
    const expense = await prisma_1.prisma.expense.findUnique({ where: { id: expenseId }, include: { approvals: true } });
    if (!expense)
        return;
    ...
    const rule = await prisma_1.prisma.approvalRule.findFirst({ where: { companyId: expense.companyId, isActive: true } });
    ...
    if (approvedByRule || approvedCount === total || total === 0) {
        await prisma_1.prisma.expense.update({ where: { id: expense.id }, data: { status: 'APPROVED' } });
        return;
    }
    if (currentIndex + 1 < total) {
        await prisma_1.prisma.expense.update({ where: { id: expense.id }, data: { currentOrderIndex: currentIndex + 1 } });
    }
    else {
